...
- 新增`docs/superpowers/specs/2026-08-31-1688-sourcing-provider-design.md`，确认将1688网页设为默认找款渠道，MuMu拼多多MVP 5.3保留为人工启用的备用渠道。
- 设计新增渠道适配层和统一候选格式；1688官方采购助手负责发起以图找货，Chrome扩展读取正常1688结果页及详情，本地Agent继续承担任务队列、Qwen判断、安全闸门和Ozon最终复价。
- 自动回填要求同款置信度不低于85、结论一致、目标规格和页面价格可复核、单件采购条件及国内运费完整；不满足时转人工，全程不自动下单。
