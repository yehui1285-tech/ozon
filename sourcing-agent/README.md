...
本模块把扩展导出的“批量店铺符合要求”Markdown转换为结构化JSON/CSV任务队列。扩展0.6.24可继续补齐主图，并基于当前页面稳定数据和“选品标签：符合要求”资格门禁，批量读取同源黑标价、计算国际运费和18%利润率最高采购成本；标签尚未加载时会等待并短暂唤醒任务页，不再直接误判为不合要求。暂未调用拼多多模拟器或云端AI。
