...
assert.match(contentSource, /selectionPending:\s*true/);
assert.match(contentSource, /选品标签仍在加载/);
...
assert.match(enrichmentSource, /live-selection-wakeup-v8/);
assert.match(backgroundSource, /snapshotResponse\.snapshot\?\.selectionPending/);
...
assert.match(enrichmentSource, /旧版核价或资格结论已作废/);
assert.match(enrichmentSource, /\["completed", "disqualified"\]\.includes/);
